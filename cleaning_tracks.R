...
files <- list.files("temp_rock_data", full.names = TRUE)
...
for (file in files) {
  ...
  # If duplicates in names are found, remove the one with the oldest date in the column "album_release_date"
  data <- data[!duplicated(data$track_name, fromLast = TRUE), ]
  
  # save the cleaned data to the new folder
  write.csv(data, paste0("cleaned_rock_data/", basename(file)), row.names = FALSE)
}
